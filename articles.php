...
// les idarticles sur la ligne commmande
  $list=[];
  // Préparer et exécuter une requête SELECT pour récupérer les id des articles  
  ...
  $PDOstatment->execute();
  // var_dump($PDOstatment->fetchAll(mode:PDO::FETCH_NUM));
  foreach($PDOstatment->fetchAll(mode:PDO::FETCH_NUM) as $tabvalue){
...
//<!-- Affichage SELECT-Afficher tous les clients de la base de données -->
    ...
    $PDOstatment=$pdo->prepare("SELECT * FROM article");
    // Exécuter la requête
    $PDOstatment-> execute();
    ...
    $lignes = $PDOstatment->fetchAll(mode:PDO::FETCH_ASSOC); 
      ...
      ?>
        <tr>
          ...
          <td> 
            ...
            <a href="updatarticle.php?idarticle=<?php echo $ligne["idarticle"]?>" class="btn btn-success">
